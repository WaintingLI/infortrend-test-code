...
def creat_new_account(customer_first_name: str,
...
def check_out() -> None :
    ...
    sleep(2)
    element = WebDriverWait(driver, 60).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR,"button#button-confirm"))
            )
...
def administration_operation_add_customer(customer_first_name: str, customer_last_name: str, customer_e_mail: str, customer_password: str) -> None:
    ...
    click_button(driver.find_element(By.CSS_SELECTOR,"div#content > div.page-header > div > div > button"))
...
if __name__ == "__main__":
    #檢查網頁是否連線正常
    response = requests.get(OPENCART_IP, timeout=10)
